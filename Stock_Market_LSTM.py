...
def get_sql_credentials():
    ...
    try:
        ...
        # Handle the case when tkinter.simpledialog is not available
        print("Warning: tkinter.simpledialog is not available. Please proved the password directly.")
        password = input("Enter your SQL Server password: ")
    return username, password

def download_stock_data(SYMBOLS, START_DATE, END_DATE, SERVER, DATABASE):
    """Download stock data."""
...
all_data = download_stock_data(SYMBOLS, START_DATE, END_DATE, SERVER, DATABASE)
...
def blend_predictions(predictions_lstm, predictions_rf, predictions_arima):
    """Blend predictions from different models."""
    blended_predictions = (predictions_lstm + predictions_rf + predictions_arima) / 3.0
...
def live_stock_feed(symbols, all_data, scaler_x_lstm, scaler_y_lstm, model_lstm, rf_model, scaler_y_rf, arima_results, num_days):
    """Live stock feed."""
    ...
    for symbol, data in zip(symbols, all_data):
        ...
        print(x_live.shape)
        # Ensure x_live has the correct shape for LSTM input
        ...
        # Specify start and end parameters for ARIMA prediction
...
model_lstm = build_lstm_model(input_shape=(4, 1))  # Assuming 4 features in the input
